...
void determine_frequency(FILE *f) {
    int c;
    while ((c = fgetc(f)) != EOF) { // freq by letter
        ++frequency[c];
        ++original_size;
    }
    for (c = 0; c < num_alphabets; ++c) // used only
        if (frequency[c] > 0)
            ++num_active;
}
...
void init() {
    ...
    leaf_index = frequency + num_alphabets - 1; // middle of freq arr
}

void allocate_tree() {
    nodes = (node_t *)
        calloc(2 * num_active, sizeof(node_t)); // full tree
    parent_index = (int *)
        calloc(num_active, sizeof(int)); // used tree only
}
...
int encode(const char* ifile, const char *ofile) {
    ...
    write_header(fout);
    build_tree();
    fseek(fin, 0, SEEK_SET); // go to 0 position
    int c;
...
int write_header(FILE *f) {
     ...
     j = sizeof(int);
     // 1 Numero de bytes en el archivo original
     while (j--) buffer[byte++] = (original_size >> (j << 3)) & 0xff;
     // 2 Numero de simbolos activos
     buffer[byte++] = (char) num_active; // save used only
     // 3 Frecuencia de los simbolos, el primer byte es el codigo del simbolo y los demas es la frecuencia.
     for (i = 1; i <= num_active; ++i) { // founding used only
         weight = nodes[i].weight;
         buffer[byte++] = (char) (-nodes[i].index - 1);
         j = sizeof(int);
         while (j--) // update buffer
             buffer[byte++] = (weight >> (j << 3)) & 0xff;
     }
     fwrite(buffer, 1, size, f); //write buffer
     free(buffer);
     return 0;
}
...
int read_header(FILE *f) {
     ...
     if (bytes_read < 1)
         return END_OF_FILE;
     byte = 0;
     // 1 obtenemos el tamano original
     original_size = buff[byte++];
     while (byte < sizeof(int))
         original_size =
             (original_size << (1 << 3)) | buff[byte++];
     // 2 obtenemos el numero de simbolos activos
     bytes_read = fread(&num_active, 1, 1, f);
     if (bytes_read < 1)
         return END_OF_FILE;

     allocate_tree(); // asignamos memoria al arbol (aun no agregamos nada)

     // 3 Obtenemos la tabla de frecuencias
